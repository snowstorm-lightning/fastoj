...
const SLUG_DETAILS: Record<string, string[]> = {
  ...
  "intersection-of-two-linked-lists": [
    "在这个数组模型里，两个链表相交表示它们拥有相同的公共后缀。返回这个公共后缀的第一个值；如果没有公共后缀，返回 null。",
  ],
  "reverse-linked-list": [
    "数组按链表从头到尾列出节点值。反转链表就是把这些值按从尾到头的顺序返回。",
  ],
  ...
  "linked-list-cycle-ii": [
    "如果存在环，入环点是从头节点沿 next 指针前进时第一个会再次进入循环的节点。在本数组模型里，它就是 pos 对应的下标。",
  ],
  ...
  "binary-tree-inorder-traversal": [
    "二叉树用带 null 占位的层序数组表示。中序遍历的访问顺序是左子树、当前节点、右子树。",
  ],
