...
讨论区树状回复/点赞/RBAC 权限和双模式样例视图分别增加了 `20260607_0009_discussion_tree_likes_rbac.py`
与 `20260607_0010_testcase_io_metadata.py`。升级后如果需要修复已导入的“智能物流定价引擎（在线学习）”
题面、题解和 ACM/Function 双视图样例，可运行：

```bash
uv run python -m backend.scripts.repair_online_least_squares_problem
```
...
如果改了 Docker、Worker、WebSocket/SSE、沙箱或真实提交流程，还要跑：
...
- Seed catalog 官方题解和增强用例：`tests/test_seed_catalog.py`
- Public solution API language fallback：`tests/test_solution_api.py`
