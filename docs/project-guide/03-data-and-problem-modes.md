...
前端 ACM starter 在 [frontend/src/lib/problemModes.ts:145](../../frontend/src/lib/problemModes.ts#L145)。后端不需要额外包装，`SubmissionService._prepare_judge_code` 会直接返回用户代码，看 [backend/services/submission_service.py:110](../../backend/services/submission_service.py#L110)。
...
包装器按函数参数逐行读取并解析。AI 出题校验中也接受一些常见 AI 生成形状，比如 JSON array 或按参数名的 object，入口在 [backend/services/problem_authoring_agent.py:36](../../backend/services/problem_authoring_agent.py#L36)。
...
- Run payload 只保留 input：[backend/services/submission_service.py:105](../../backend/services/submission_service.py#L105)
- 生成自定义运行用例：[backend/worker/tasks/judge_task.py:310](../../backend/worker/tasks/judge_task.py#L310)
- 用官方解法生成 expected output：[backend/worker/tasks/judge_task.py:326](../../backend/worker/tasks/judge_task.py#L326)

## 隐藏用例的数据边界

隐藏用例可以保存在数据库中，管理员可以管理，但普通提交详情不能返回隐藏输入、期望输出或实际输出。

JudgeTask 写 `TestCaseResult` 时对隐藏用例做了空值处理：[backend/worker/tasks/judge_task.py:216](../../backend/worker/tasks/judge_task.py#L216)。

普通用户获取提交详情时还会过滤隐藏结果：[backend/api/submissions/__init__.py:83](../../backend/api/submissions/__init__.py#L83)。
...
- 隐藏结果写入保护：[backend/worker/tasks/judge_task.py:216](../../backend/worker/tasks/judge_task.py#L216)
